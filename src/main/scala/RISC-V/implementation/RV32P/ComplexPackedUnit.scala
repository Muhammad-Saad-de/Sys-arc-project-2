...
class ComplexPackedUnit extends AbstractExecutionUnit {
  ...
  // ---------------------------------------------------------------------------
  // Bulletproof Shift Decoding via Base + Modifier Bits
  // ---------------------------------------------------------------------------
  val isShift = (opcode === OP_IMM_32 || opcode === OP_32) && (funct3 === "b010".U)
  ...
  val isPNCLIPI_B  = (opcode === OP_IMM_32 || opcode === OP_32) && isClipFunct3 && (funct7 === "b0110000".U)
  val isPNCLIPI_H  = (opcode === OP_IMM_32 || opcode === OP_32) && isClipFunct3 && (funct7 === "b0110001".U)
  val isPNCLIPRI_B = (opcode === OP_IMM_32 || opcode === OP_32) && isClipFunct3 && (funct7 === "b0111000".U)
  val isPNCLIPRI_H = (opcode === OP_IMM_32 || opcode === OP_32) && isClipFunct3 && (funct7 === "b0111001".U)

  val isPM2ADD_H    = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1000000".U)
  val isPM2ADDU_H   = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1010000".U)
  val isPM2ADDSU_H  = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1110000".U)
  val isPM2ADDA_H   = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1000100".U)
  val isPM2ADDAU_H  = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1010100".U)
  val isPM2ADDASU_H = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1110100".U)
  
  val isPM2ADD_HX   = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1001000".U)
  val isPM2ADDA_HX  = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1001100".U)
  val isPM2SADD_H   = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1100010".U)
  val isPM2SADD_HX  = (opcode === OP_32) && (funct3 === "b101".U) && (funct7 === "b1101010".U)
  ...
  val isMultiCycle = isPM2ADDA_H || isPM2ADDASU_H || isPM2ADDAU_H || isPM2ADDA_HX
  ...
  switch(state) {
    is(sIdle) {
      when(isKnown) {
        ...
        when(isSingleCycle) {
          ...
          val sat_sum_s = Mux(sum_s > max32, max32, Mux(sum_s < min32, min32, sum_s))
          val sat_sum_x = Mux(sum_x > max32, max32, Mux(sum_x < min32, min32, sum_x))

          val isArith = isPSSHA_HS || isPSSHAR_HS
          val isRnd = isPSSHAR_HS || isPSSHLR_HS

          def doShift(value: UInt, shamtRaw: UInt): UInt = {
            val shamt = shamtRaw(4, 0)
            val isRight = shamt(4)
            val shAmtMag = Mux(isRight, (~shamt + 1.U)(4, 0), shamt(4, 0))
            ...
            val rightShifted = Mux(isArith, rightShiftedArith, rightShiftedLogic)
            
            val valExt = Cat(0.U(1.W), value)
            val discardedBit = Mux(shAmtMag === 0.U, 0.U, (valExt >> (shAmtMag - 1.U))(0))
            val roundedRight = rightShifted + Mux(isRnd && isRight, discardedBit, 0.U)
            
            val max16 = 32767.S(32.W)
            val min16 = -32768.S(32.W)
            val satLeft = Mux(leftShiftedSInt > max16, max16.asUInt, Mux(leftShiftedSInt < min16, min16.asUInt, leftShiftedSInt.asUInt))(15, 0)
            
            val leftResult = Mux(isArith, satLeft, logicLeft)
            Mux(isRight, roundedRight(15, 0), leftResult)
          }
          ...
          def clipShiftB(x: SInt): UInt = {
            ...
            val roundBit = Mux(clip_shamt === 0.U, 0.S, Mux((x >> (clip_shamt - 1.U))(0), 1.S, 0.S))
            val rounded = shifted + Mux(isClipRndB, roundBit, 0.S)
            val maxB = 127.S
            val minB = -128.S
            Mux(rounded > maxB, maxB.asUInt(7,0), Mux(rounded < minB, minB.asUInt(7,0), rounded.asUInt(7,0)))
          }
          ...
          def clipShiftH(x: SInt): UInt = {
            ...
            val maxH = 32767.S
            val minH = -32768.S
            Mux(rounded > maxH, maxH.asUInt(15,0), Mux(rounded < minH, minH.asUInt(15,0), rounded.asUInt(15,0)))
          }
          ...
          val singleResult = MuxCase(0.U(32.W), Seq(
            ...
            (isPNCLIPI_B || isPNCLIPRI_B) -> Cat(clipB_h3, clipB_h2, clipB_h1, clipB_h0),
    ...
    is(sReadRD) {
      ...
      val accumulateResult = MuxCase(0.U(32.W), Seq(
